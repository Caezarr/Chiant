...
`box-evidence-pack` regroupe ensuite les rapports terrain et `/var/lib/boring/events.jsonl` dans `reports/evidence-pack.json` pour audit, partage ou support. Les rapports doivent exposer `passed=true`; le journal runtime doit etre du JSONL valide, contenir au moins un `heartbeat` et ne contenir aucun evenement bloquant. Chaque item inclut `format`, `size_bytes` et `sha256`; si un rapport ou le journal est modifie apres generation du pack, son empreinte ne correspond plus.
