...
```bash
uv run boring box-ready --allow-dry-run --allow-missing-autopay-smoke --allow-missing-edge-export --allow-missing-charge-validation --allow-missing-network-recovery --allow-missing-notification-webhook --allow-missing-notification-test --allow-missing-runtime-event-log --min-burn-in-hours 2
```
