...
def test_production_readiness_rejects_restarted_systemd_runtime(tmp_path: Path):
    artifacts = _write_ready_artifacts(tmp_path)
    payload = json.loads(artifacts["systemd"].read_text())
    payload["n_restarts"] = 3
    artifacts["systemd"].write_text(json.dumps(payload))
...
def test_production_readiness_requires_systemd_restart_count(tmp_path: Path):
    artifacts = _write_ready_artifacts(tmp_path)
    payload = json.loads(artifacts["systemd"].read_text())
...
def test_production_readiness_fails_when_vision_eval_has_no_negative_coverage(
    tmp_path: Path,
):
    artifacts = _write_ready_artifacts(tmp_path)
    payload = json.loads(artifacts["vision_eval"].read_text())
...
def test_production_readiness_fails_when_vision_eval_frame_coverage_is_inconsistent(
    tmp_path: Path,
):
    artifacts = _write_ready_artifacts(tmp_path)
    payload = json.loads(artifacts["vision_eval"].read_text())
...
def test_production_readiness_fails_when_vision_eval_has_invalid_labels(tmp_path: Path):
    artifacts = _write_ready_artifacts(tmp_path)
    payload = json.loads(artifacts["vision_eval"].read_text())
...
def test_production_readiness_rejects_autopay_smoke_for_unverified_active_amount(
    tmp_path: Path,
):
    artifacts = _write_ready_artifacts(tmp_path)
    payload = json.loads(artifacts["autopay_smoke"].read_text())
...
def test_production_readiness_requires_autopay_smoke_active_amount(tmp_path: Path):
    artifacts = _write_ready_artifacts(tmp_path)
    payload = json.loads(artifacts["autopay_smoke"].read_text())
...
def test_production_readiness_rejects_autopay_smoke_for_unverified_active_duration(
    tmp_path: Path,
):
    artifacts = _write_ready_artifacts(tmp_path)
    payload = json.loads(artifacts["autopay_smoke"].read_text())
        ...
        json.dumps(
            {
                ...
                "active_session_amount_cents": 120 if autopay_smoke_passed else 0,
                "amount_verified": autopay_smoke_passed,
                "duration_minutes": 15,
                "active_session_duration_minutes": 15,
                "duration_verified": autopay_smoke_passed,
                "lat": 50.6371,
                "lon": 3.0633,
