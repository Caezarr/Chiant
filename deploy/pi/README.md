...
`box-burn-in` ecrit `report.json` et `samples.jsonl`. Avant beta terrain, garder un rapport avec `passed=true`, `camera_failures=0`, `network_failures=0`, `charging_seen=true`, `discharging_seen=true`, `max_temp_c` present et sous `THERMAL_CRITICAL_C`, pas de batterie faible ou critique et pas de temperature critique.
