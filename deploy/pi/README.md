...
`box-ready` est le gate final : pour une vraie promesse 10h, relancer avec `--min-burn-in-hours 10` sur un burn-in voiture complet et un `reports/autopay-smoke.json` issu d'un paiement reel minimal. Les rapports critiques doivent etre recents; par defaut `BOX_READINESS_MAX_REPORT_AGE_HOURS=72`.
