...
`box-power-check` ecrit `reports/power-check.json`. La jauge batterie/UPS doit etre visible dans `/sys/class/power_supply`, avec un pourcentage et un statut charge/decharge exploitables. L'autonomie estimee est calculee sur l'energie disponible au pourcentage courant, pas sur la capacite nominale pleine. Le rapport garde `ESTIMATED_DRAW_WATTS` et `REQUIRED_RUNTIME_HOURS`; relance `box-power-check` apres tout changement de conso estimee ou de promesse runtime, sinon `box-ready` refuse la preuve. Si la jauge disparait pendant le runtime, la box journalise `battery_sensor_missing`, notifie l'utilisateur et bloque l'autopaiement jusqu'au retour de la jauge.
