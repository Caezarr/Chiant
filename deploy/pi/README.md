...
`box-evidence-pack` regroupe les rapports terrain et `/var/lib/boring/events.jsonl` dans `reports/evidence-pack.json`; sauf profil hardware, chaque rapport doit exposer `passed=true`. Le journal runtime doit etre du JSONL valide, contenir au moins un `heartbeat` et ne contenir aucun evenement bloquant. Le pack inclut aussi `format`, `size_bytes` et `sha256` pour chaque preuve presente.
