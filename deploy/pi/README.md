...
`box-network-check` ecrit `reports/network-check.json`. La cible `NETWORK_PROBE_TARGET` doit etre joignable et `NETWORK_RECOVERY_COMMAND` doit etre configure; le rapport garde la commande exacte, et `box-ready` refuse une preuve generee avec une commande de recovery differente de l'env courant.
