...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tests-417%2F417-brightgreen" alt="Tests">
</p>
...
make test                     # 428 tests doivent passer
...
uv run boring box-evidence-pack --model models/best.pt --output reports/evidence-pack.json  # refuse les rapports >72h
uv run boring contest-fps --subject "FPS-X" --reason "Test"  # smoke test contestation
```

Pour `box-ready` en mode prod, lance aussi `autopay-smoke` avec `PAYMENT_DRY_RUN=false`, configure `NETWORK_RECOVERY_COMMAND`, `BORING_NOTIFY_WEBHOOK_URL` ou `NTFY_WEBHOOK_URL`, puis lance `box-runtime-checks`, `box-notify-test` et `box-systemd-check`; sans preuve d'autopaiement reel minimal, camera runtime, position runtime, reseau joignable, jauge batterie runtime, recovery reseau, notification 2xx horodatee et service systemd actif, le boitier n'est pas installable.
...
- Tests pytest (428/428), CI GitHub Actions, pre-commit hooks
